const MaternityPatient = require('../model/maternityDB');

const getAllPatients = async (req, res) => {
    const patients = await MaternityPatient.find();
...
const addNewPatient = async (req, res) => {
    ...
    const newPatient = await MaternityPatient.create({
...
const updatePatient = async (req, res) => {
    ...
    let foundPatient = await MaternityPatient.findOne({ _id: req.body.id}).exec();
...
const deletePatient = async (req, res) => {
    ...
    const foundPatient = await MaternityPatient.findOne({ _id: req.body.id}).exec();
    if (!foundPatient) {
        return res.status(404).json({ "message" : `ID ${req.body.id} does not match any patient in the ward`});
    }

    const result = await MaternityPatient.deleteOne({ _id: req.body.id });
...
const getPatient = async (req, res) => {
    ...
    const foundPatient = await MaternityPatient.findOne({ _id : req.params.id}).exec();
    if (!foundPatient) {
